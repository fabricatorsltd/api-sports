...
public sealed class MmaSampleResponseTests
{
    ...
    {
        (bool found, string? json) = SampleJsonLoader.TryLoadMmaSample(endpoint.Definition.SamplePath);
    ...
    {
        MmaJsonSerializerContext context = MmaJsonSerializerContext.Default;
        ...
        [
            ...
            new SampleEndpoint<string[]>(GetDefinition(typeof(CategoriesClient), nameof(CategoriesClient.GetAsync)), context.ApiResponseStringArray),
            new SampleEndpoint<Team[]>(GetDefinition(typeof(TeamsClient), nameof(TeamsClient.GetAsync)), context.ApiResponseTeamArray),
            new SampleEndpoint<Fighter[]>(GetDefinition(typeof(FightersClient), nameof(FightersClient.GetAsync)), context.ApiResponseFighterArray),
            new SampleEndpoint<FighterRecord[]>(GetDefinition(typeof(FightersClient), nameof(FightersClient.GetRecordsAsync)), context.ApiResponseFighterRecordArray),
            new SampleEndpoint<Fight[]>(GetDefinition(typeof(FightsClient), nameof(FightsClient.GetAsync)), context.ApiResponseFightArray),
            new SampleEndpoint<FightResult[]>(GetDefinition(typeof(FightsClient), nameof(FightsClient.GetResultsAsync)), context.ApiResponseFightResultArray),
            new SampleEndpoint<FightStatistics[]>(GetDefinition(typeof(FightsClient), nameof(FightsClient.GetFighterStatisticsAsync)), context.ApiResponseFightStatisticsArray),
    ...
    private static EndpointDefinition GetDefinition(Type clientType, string methodName)
    {
        ...
        if (!MmaEndpointCatalog.EndpointByMethod.TryGetValue(key, out EndpointDefinition? definition))
        {
