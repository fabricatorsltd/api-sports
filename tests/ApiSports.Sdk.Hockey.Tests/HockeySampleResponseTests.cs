...
public sealed class HockeySampleResponseTests
{
    ...
    {
        (bool found, string? json) = SampleJsonLoader.TryLoadHockeySample(endpoint.Definition.SamplePath);
    ...
    {
        HockeyJsonSerializerContext context = HockeyJsonSerializerContext.Default;
    ...
    private static EndpointDefinition GetDefinition(Type clientType, string methodName)
    {
        ...
        if (!HockeyEndpointCatalog.EndpointByMethod.TryGetValue(key, out EndpointDefinition? definition))
        {
