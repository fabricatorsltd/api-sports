...
using ApiSports.Sdk.Volleyball.Tests.Helpers;
using Xunit;

namespace ApiSports.Sdk.Volleyball.Tests;

public sealed class VolleyballSampleResponseTests
{
    ...
    {
        (bool found, string? json) = SampleJsonLoader.TryLoadVolleyballSample(endpoint.Definition.SamplePath);
    ...
    {
        VolleyballJsonSerializerContext context = VolleyballJsonSerializerContext.Default;
    ...
    private static EndpointDefinition GetDefinition(Type clientType, string methodName)
    {
        ...
        if (!VolleyballEndpointCatalog.EndpointByMethod.TryGetValue(key, out EndpointDefinition? definition))
        {
