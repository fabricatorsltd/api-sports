...
using ApiSports.Sdk.Basketball.Tests.Helpers;
using Xunit;

namespace ApiSports.Sdk.Basketball.Tests;

public sealed class BasketballSampleResponseTests
{
    ...
    {
        (bool found, string? json) = SampleJsonLoader.TryLoadBasketballSample(endpoint.Definition.SamplePath);
    ...
    {
        BasketballJsonSerializerContext context = BasketballJsonSerializerContext.Default;
        ...
        [
            ...
            new SampleEndpoint<JsonElement[]>(GetDefinition(typeof(SeasonsClient), nameof(SeasonsClient.GetAsync)), context.ApiResponseJsonElementArray),
    ...
    private static EndpointDefinition GetDefinition(Type clientType, string methodName)
    {
        ...
        if (!BasketballEndpointCatalog.EndpointByMethod.TryGetValue(key, out EndpointDefinition? definition))
        {
