...
public sealed class BaseballSampleResponseTests
{
    ...
    {
        (bool found, string? json) = SampleJsonLoader.TryLoadBaseballSample(endpoint.Definition.SamplePath);
    ...
    {
        BaseballJsonSerializerContext context = BaseballJsonSerializerContext.Default;
    ...
    private static EndpointDefinition GetDefinition(Type clientType, string methodName)
    {
        ...
        if (!BaseballEndpointCatalog.EndpointByMethod.TryGetValue(key, out EndpointDefinition? definition))
        {
