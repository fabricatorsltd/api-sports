...
using ApiSports.Sdk.Handball.Tests.Helpers;
using Xunit;

namespace ApiSports.Sdk.Handball.Tests;

public sealed class HandballSampleResponseTests
{
    ...
    {
        (bool found, string? json) = SampleJsonLoader.TryLoadHandballSample(endpoint.Definition.SamplePath);
    ...
    {
        HandballJsonSerializerContext context = HandballJsonSerializerContext.Default;
    ...
    private static EndpointDefinition GetDefinition(Type clientType, string methodName)
    {
        ...
        if (!HandballEndpointCatalog.EndpointByMethod.TryGetValue(key, out EndpointDefinition? definition))
        {
